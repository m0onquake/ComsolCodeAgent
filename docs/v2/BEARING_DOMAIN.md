...
延续路径显式区分两种 profile。`engineering_preview` 使用一个原生求解器序列和按目标载荷
缩放的稀疏点（启动点、1%、10%、100%），相对容差最多放宽到 `1e-2`，默认总预算
`5400 s`；其目标是得到本次真实目标步应力，而不是证明逐滚子力平衡。`strict_verified`
保留更密的方向感知延续、最多两个继承块和用户指定的严格容差。profile、实际容差和延续点
必须写入运行结果；预览失败后才允许回退严格路径，二者证据不得混写。
