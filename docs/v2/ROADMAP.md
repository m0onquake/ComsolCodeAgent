...
- 2026-09-01 将默认工程预览从旧严格适配器的二次重求解中拆出：一次稀疏目标缩放延续后
  直接读取本次解、导出原生图和 solved MPH。新鲜 10 滚子 `+X/1 N` 在 154.26 秒通过
  preview（84529.135 Pa）；8 滚子小尺寸 `+Y/0.2 N` 在 3622.66 秒返回
  `NON_CONVERGENCE`，因此宽参数方向矩阵仍未完成；
